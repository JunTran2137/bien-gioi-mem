...
/* ------------------------------ RoleGate ----------------------------- */
function PopupShell({ title, subtitle, children, onClose }: { title: string; subtitle?: string; children: React.ReactNode; onClose?: () => void }) {
  return (
    <div className="min-h-screen flex items-center justify-center p-4">
      <div className="w-full max-w-lg bg-bg rounded-2xl border border-border shadow-2xl p-6 md:p-8 relative">
        {onClose && (
          <button
            onClick={onClose}
            aria-label="Đóng"
            className="absolute top-4 right-4 rounded-full p-1.5 text-muted hover:text-text hover:bg-border/50 transition"
          >
...
function RoleGate({ creating, onHost }: { creating: boolean; onHost: () => void }) {
  ...
  return (
    <PopupShell
      title="Đại Sứ Tri Thức — Cờ Tỷ Phú"
      subtitle="Bạn tham gia với vai trò nào?"
    ...
    >
      {mode === 'choose' ? (
        <>
          ...
          <div className="mt-5 rounded-xl border border-border bg-primary-soft p-4 flex items-center justify-between gap-3">
            <div>
              <p className="text-sm font-semibold text-text">📜 Luận Giải</p>
