...
const WIN_SCORE = 300;            // group score for winning a match
const FINALIST_BONUS = 200;       // extra for reaching the grand final
const VOTER_ACCURACY_BONUS = 50;  // personal score for voting for the winner
...
function createRoom(roomCode, hostUid) {
  ...
  if (existing) { if (hostUid) existing.hostUid = hostUid; return existing; }
  const room = {
    roomCode,
    hostUid,
...
function joinRoom(roomCode, player) {
  ...
  if (!room.groups.has(gid)) room.groups.set(gid, { groupId: gid, groupName: gname, score: 0 });
  return room;
}
...
function publicGroups(room) {
  return Array.from(room.groups.values()).map(g => ({ ...g })).sort((a, b) => b.score - a.score);
}
...
function startMatch(room, io) {
  ...
  room.match.deadline = Date.now() + PREP_SECONDS * 1000;
  emitState(room, io);
  room.timer = setTimeout(() => nextTurn(room, io), PREP_SECONDS * 1000);
}
...
function nextTurn(room, io) {
  ...
  emitState(room, io);
  room.timer = setTimeout(() => nextTurn(room, io), TURN_SECONDS * 1000);
}
...
function startVoting(room, io) {
  ...
  room.match.deadline = Date.now() + VOTE_SECONDS * 1000;
  emitState(room, io);
  room.timer = setTimeout(() => resolveMatch(room, io), VOTE_SECONDS * 1000);
}
...
function resolveMatch(room, io) {
  ...
  // award match-win score
  const winnerGroup = room.groups.get(winner);
  if (winnerGroup) winnerGroup.score += WIN_SCORE;
  ...
  try {
    ...
    const tx = db.transaction(() => {
      for (const [voterUid, votedGid] of m.votes) {
        ...
        if (votedGid === winner) incUser.run(VOTER_ACCURACY_BONUS, voterUid);
      }
    });
    tx();
  ...
  room.matchIdx++;
  room.timer = setTimeout(() => startMatch(room, io), RESULT_SECONDS * 1000);
}
  ...
  try {
    ...
    const tx = db.transaction(() => {
      for (const g of room.groups.values()) {
        if (g.groupId && g.groupId !== '__no_group__' && g.score > 0) {
          incGroup.run(g.score, g.groupId);
        }
      }
    });
    tx();
