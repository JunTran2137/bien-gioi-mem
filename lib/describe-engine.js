...
//              think it describes; their phone camera OCR-reads the card name
...
function publicState(room) {
  return {
    ...
    scribes: Object.fromEntries(room.scribes || new Map()),
...
function beginReveal(roomCode, io) {
  ...
  // Build round-robin queue: desc#1 of each group, then desc#2, then desc#3...
  // Groups sorted by name (numeric-aware).
  const groupLists = [];
  for (const [gid, list] of room.submissions) {
    const group = room.groups.get(gid);
    const gname = group ? group.groupName : 'Nhóm';
    groupLists.push({ gname, descs: list.map(s => ({
      authorGroupId: gid, authorGroupName: gname, text: s.text, cardId: s.cardId
    })) });
  }
  groupLists.sort((a, b) => a.gname.localeCompare(b.gname, 'vi', { numeric: true }));
  // Interleave: round 0 takes desc[0] from every group, round 1 takes desc[1], etc.
  const queue = [];
  const maxLen = groupLists.reduce((m, g) => Math.max(m, g.descs.length), 0);
  for (let round = 0; round < maxLen; round++) {
    for (const { descs } of groupLists) {
      if (round < descs.length) queue.push(descs[round]);
    }
  }
