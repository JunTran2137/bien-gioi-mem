...
const describeOcr = require('./describe-ocr');
const { getDb } = require('./db');

/** roomCode -> Map<groupId, socketId> — tracks which socket holds each group's camera */
const camSessions = new Map();
/** `${roomCode}:${groupId}` -> last relay timestamp — server-side FPS cap */
const camLastRelay = new Map();
const CAM_RELAY_MIN_MS = 33; // max ~30fps relay regardless of phone send rate
...
function initSocketHandlers(io) {
  io.on('connection', socket => {
    ...
    socket.on('dgcam:frame', ({ roomCode, groupId, jpeg }) => {
      ...
      // Server-side rate limit: drop frames arriving faster than CAM_RELAY_MIN_MS
      const rateKey = `${roomCode}:${groupId}`;
      const now = Date.now();
      if (now - (camLastRelay.get(rateKey) || 0) < CAM_RELAY_MIN_MS) return;
      camLastRelay.set(rateKey, now);
      // Relay only to non-camera sockets (skip other camera phones)
      // compress(false): JPEG is already compressed, skip gzip
      ...
      for (const sid of room) {
        ...
        if (!s) continue;
        if (s.data?.dgCamGroup) continue; // skip other camera phones
        // volatile: never queue frames for a slow host client. If it can't keep
        // up, drop — the next frame is only ~200ms away and staying real-time
        // matters more than delivering every single frame.
        s.volatile.compress(false).emit('dg:cameraFrame', { groupId, jpeg });
      }
    });

    // Sharp, low-rate frame for SERVER-SIDE OCR only (not relayed to the grid).
    // The device sends ~1fps at a higher resolution so the card name is legible.
    socket.on('dgcam:ocrframe', ({ roomCode, groupId, jpeg }) => {
      if (!roomCode || !groupId || !jpeg) return;
      describeOcr.submitFrame(roomCode, groupId, jpeg, io);
    });

    socket.on('dgcam:tiles', ({ roomCode, groupId, tiles }) => {
      if (!roomCode || !groupId) return;
      const list = Array.isArray(tiles) ? tiles : [];
      // Relay only to non-camera sockets (camera phones don't need other groups' tiles)
      const tileRoom = io.sockets.adapter.rooms.get(roomCode);
      if (tileRoom) {
        for (const sid of tileRoom) {
          if (sid === socket.id) continue;
          const s = io.sockets.sockets.get(sid);
          if (!s || s.data?.dgCamGroup) continue; // skip other camera phones
          s.emit('dg:cameraTiles', { groupId, tiles: list });
        }
      }
