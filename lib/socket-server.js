...
function initSocketHandlers(io) {
  io.on('connection', socket => {
    ...
    socket.on('debate:argument', ({ roomCode, uid, text }) => {
      const result = debate.submitArgument(roomCode, uid, text);
      if (result) {
        io.to('debate:' + roomCode).emit('debate:argument', result);
      }
    });

    socket.on('debate:challenge', ({ roomCode, uid, text }) => {
      const result = debate.submitChallenge(roomCode, uid, text);
      if (result) {
        io.to('debate:' + roomCode).emit('debate:challenge', result);
      }
    });

    socket.on('debate:response', ({ roomCode, uid, text }) => {
      const result = debate.submitResponse(roomCode, uid, text);
      if (result) {
        io.to('debate:' + roomCode).emit('debate:response', result);
      }
    });
    ...
    socket.on('debate:react', ({ roomCode, uid, kind }) => {
      ...
      if (result) {
        io.to('debate:' + roomCode).emit('debate:reaction', result);
      }
    });
    ...
    socket.on('debate:vote', ({ roomCode, uid, votedGroupId }) => {
      ...
      if (tally) {
        io.to('debate:' + roomCode).emit('debate:voteUpdate', tally);
      }
    });
    ...
    socket.on('dgcam:join', ({ roomCode, groupId, groupName }, ack) => {
      ...
      ack && ack({ ok: true, roomCode, groupId });
    });
    ...
    socket.on('disconnect', () => {
      ...
      if (socket.data && socket.data.dgCamRoom) {
        io.to(socket.data.dgCamRoom).emit('dg:cameraStatus', { groupId: socket.data.dgCamGroup, connected: false });
      }
